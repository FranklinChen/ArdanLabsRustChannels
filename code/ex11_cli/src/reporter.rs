...
pub fn reporter_thread(report_rx: flume::Receiver<Report>) {
    loop {
        match report_rx.recv() {
            Ok(Report::Producer(id, messages_per_second)) => {
                ...
                PRODUCER_PERFORMANCE[id].store(
                    messages_per_second as u32,
